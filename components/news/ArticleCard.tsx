        ...
        <Link href={newsHref} className="block">
          <div className="relative aspect-[16/9] overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
              {article.isBreaking && (
                <span className="mb-2 inline-block rounded bg-news-accent px-2 py-0.5 text-xs font-bold uppercase text-white">
  ...
  if (variant === "horizontal") {
    return (
      <article className={cn("group flex gap-4", className)}>
        <Link
          href={newsHref}
          className="relative block h-24 w-32 flex-shrink-0 overflow-hidden rounded-lg sm:h-28 sm:w-40"
        >
          {article.featuredImage ? (
            <Image
              src={article.featuredImage}
        ...
        <div className="flex flex-1 flex-col justify-center">
          ...
          <Link href={newsHref}>
            <h3 className="mt-1 line-clamp-2 text-base font-bold leading-snug group-hover:text-brand-600 transition-colors">
              {article.title}
            </h3>
          </Link>
          <time
            dateTime={article.publishedAt?.toISOString()}
            className="mt-1 text-xs text-news-muted"
          >
            {formatRelativeDate(article.publishedAt, locale)}
          </time>
        </div>
    ...
    <article className={cn("group", className)}>
      <Link href={newsHref} className="block">
        <div className="relative aspect-[16/10] overflow-hidden rounded-lg">
          ...
          {article.isBreaking && (
            <span className="absolute start-2 top-2 rounded bg-news-accent px-2 py-0.5 text-xs font-bold uppercase text-white">
              {dict.article.breaking}
            </span>
          )}
        </div>
        <div className="mt-3">
          ...
          <h3 className="mt-1 line-clamp-2 text-lg font-bold leading-snug group-hover:text-brand-600 transition-colors">
            {article.title}
          </h3>
          {article.excerpt && (
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-news-muted">
            <span>{article.author.name}</span>
            <span>•</span>
            <time dateTime={article.publishedAt?.toISOString()}>
              {formatRelativeDate(article.publishedAt, locale)}
